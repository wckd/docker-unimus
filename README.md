...
You can use HSQL (local - file-based) or MySQL (with another container) for the backend DB.

You can run the unimus with 2 methods:
...
Configuration and HSQL database files are in `/etc/unimus` folder in the container.

### with MySQL

Create a Docker network and start your MySQL container for Unimus:

```sh
docker network create unimus-net
docker run -tid --name=unimus-db --network=unimus-net -v /srv/unimus/db:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=supersecret -e MYSQL_DATABASE=unimus -e MYSQL_USER=unimus -e MYSQL_PASSWORD=secret mariadb:lts
```

Start your Unimus:

```sh
docker run -tid --name=unimus --network=unimus-net -p 8085:8085 -v /srv/unimus/config:/etc/unimus/ wckd0/unimus
```
...
Check these pages:
  ...
  - https://github.com/wckd/docker-unimus/issues/14

Add some extra parameters into the docker-compose file for the proxy connection. Example:
```yaml
